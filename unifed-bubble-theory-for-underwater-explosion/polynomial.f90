    ...
    type legendre
        integer order
        real,allocatable:: a(:),da(:),dda(:),ddda(:)   ! coefficients for the 
                                        ! polynomial and its 1st, 2nd and 3rd derivative
    contains
    ...
    if(present(x1).ne.present(x2))then
    ...
    this%order = n
    allocate(this%a(0:n),this%da(0:n-1),this%dda(0:n-2),this%ddda(0:n-3))
