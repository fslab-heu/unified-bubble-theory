    ...
    !write(name,*) id
    !open(this%fid,file='./output/bubble.dat')
    ...
    function collect_induced_field(pos,time) result(pressure)
    use global
    use boundary
    implicit none
    integer i
    ...
    ! calculate dvel
    dvel = - sub / dis**3.0*(&
        this%bhis(n,2)/c*(dis-this%bhis(n,2))*(this%bhis(n,6)+0.5*this%bhis(n,3)**2.0)+this%bhis(n,2)**2.0*this%bhis(n,3)) + &
        sub / dis**3.0*(&
        this%bhis(m,2)/c*(dis-this%bhis(m,2))*(this%bhis(m,6)+0.5*this%bhis(m,3)**2.0)+this%bhis(m,2)**2.0*this%bhis(m,3))
    dvel =dvel/tinc
    !
    ...
    subroutine induced_flow_bound(this,pos,vel,dvel,dphi,ddphi,grad_dphi,stg,impacted)
    use global
    use boundary
    implicit none
    class(sbubble):: this
    ...
    alpha = bound1%alpha
    center=this%center
    center = center-(2*dot_product(bound1%norm,center+bound1%b))*bound1%norm
    sub = center-pos
    dis = length(sub)
    dir = - sub / dis
    tint = time - (dis-this%r)/c
    ...
    if(t+dt>this%ts)then
        ...
        dat2(3:4) = [enthalpy(this,dat2(1)),denthalpy(this,dat2(1),dat2(2))]
    ...
    solve = 0
    if(imigration)then
        solve(2:4) = (-144.0 * uref * rho *               &
            (Ca - 1 / 24.0) * dR ** 3.0 +                 &
            216.0 * uref * rho * (Ca -                    &
            1.0 / 12.0) * C * dR ** 2.0 +                   &
            (5.0 * uref ** 3.0 * rho - 72.0 *                 &
            (C ** 2.0 * Ca - H / 6.0) * rho * uref          &
            + 9.0 * (Cd * ss * rho + 8.0 / 3.0 * R * NPa) * C)&
            * dR + 12.0 * R * uref * dH * rho + 3.0         &
            * C * uref ** 3.0 * rho + 12.0 * C * uref       &
            * H * rho - 9.0 * (Cd * ss * rho + 8.0 /        &
            3.0 * R * NPa) * C ** 2.0) / (C ** 2.0 * Ca       &
            - 3.0 * C * Ca * dR + 2.0 * Ca * dR ** 2.0 -      &
            7.0 / 12.0 * uref ** 2.0) / R / rho / 24.0
    endif
    solve(1) = ((-18 * dR ** 2 * C + 3 * C * u2 + 14    &
     * dot_product(uref,solve(2:4)) * R + 6 * dR ** 3   &
      + 5 * dR * u2 + 12 * C * H + 12 * dH * R +        &
      12 * H * dR) / R / (C - dR)) / 12
    ...
    integer i
    real dphi,vel(3),ddphi,dphi1,vel1(3),grad_phi(3),dvel(3)
    dphi=0;
    vel=uamb
    
    do i=1,nbubble
        call induced_flow(bubbles(i),pos,vel1,dvel,dphi1,ddphi,grad_phi,1)
        vel=vel+vel1
        dphi=dphi+dphi1
    enddo
    if(ibound)then
        do i=1,nbubble
            call induced_flow_bound(bubbles(i),pos,vel1,dvel,dphi1,ddphi,grad_phi,1)
            vel=vel+vel1
            dphi=dphi+dphi1
        enddo
    endif
    pressure_at_position = -rho*(dphi + 0.5*dot_product(vel,vel)) &
